...
fn start_bridge_with_recording(path: String, port: u16) -> Result<String, String> {
    ...
    std::thread::spawn(move || {
        rt.block_on(async {
            ...
            fundamentals_bridge::start_server_with_recording(recording_path, port, false, false).await;
        });
    });
    ...
    if default_recording.exists() {
        ...
        std::thread::spawn(move || {
            rt_clone.block_on(async {
                // Launch the bridge with the recording
                fundamentals_bridge::start_server_with_recording(recording_path, default_port, false, false).await;
            });
        });
    } else {
        // Log that no recording file was found
        eprintln!("No recording file found at {:?}. Bridge server not started automatically.", default_recording);
        eprintln!("You can start the bridge manually using the start_bridge_with_recording command.");
    }
